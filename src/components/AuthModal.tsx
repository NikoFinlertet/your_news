  ...
  useEffect(() => {
    async function checkProviders() {
      try {
        ...
        console.error('Ошибка при получении списка провайдеров:', err);
      }
    }
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'github') => {
    try {
      ...
      if (!availableProviders.includes(provider)) {
        throw new Error('Этот способ входа временно недоступен. Пожалуйста, попробуйте другой способ или обратитесь в поддержку.');
      }
      ...
      if (error) {
        if (error.message.includes('provider is not enabled')) {
          ...
          throw new Error('Неверные учетные данные. Пожалуйста, проверьте введенные данные и попробуйте снова.');
    ...
    <div className="auth-modal-overlay" onClick={onClose}>
      <div className="auth-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="auth-modal-content">
          <h2 className="auth-modal-title">Вход в систему</h2>
